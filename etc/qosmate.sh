...
# 4 - family (ipv4|ipv6)
add_tc_filter() {
    local dec_id dsfield class_id hex_match proto prio match_str \
        dev="$1" \
        class_enum="$2" \
        family="$3"

    case "$class_enum" in
        cs0|CS0) dec_id=0 dsfield=0x00 class_id=1:13 hex_match=0x0000 ;; # 0 -> Default
        ef|EF) dec_id=46 dsfield=0xb8 class_id=1:11 hex_match=0x0B80 ;; # 46
        cs1|CS1) dec_id=8 dsfield=0x20 class_id=1:15 hex_match=0x0200 ;; # 8
        cs2|CS2) dec_id=16 dsfield=0x40 class_id=1:14 hex_match=0x0400 ;; # 16
        cs4|CS4) dec_id=32 dsfield=0x80 class_id=1:12 hex_match=0x0800 ;; # 32
        cs5|CS5) dec_id=40 dsfield=0xa0 class_id=1:11 hex_match=0x0A00 ;; # 40
        cs6|CS6) dec_id=48 dsfield=0xc0 class_id=1:11 hex_match=0x0C00 ;; # 48
        cs7|CS7) dec_id=56 dsfield=0xe0 class_id=1:11 hex_match=0x0E00 ;; # 56
        af11|AF11) dec_id=10 dsfield=0x28 class_id=1:14 hex_match=0x0280 ;; # 10
        af41|AF41) dec_id=34 dsfield=0x88 class_id=1:12 hex_match=0x0880 ;; # 34
        af42|AF42) dec_id=36 dsfield=0x90 class_id=1:12 hex_match=0x0900 ;; # 36
        *) # TODO: throw an error
    esac
